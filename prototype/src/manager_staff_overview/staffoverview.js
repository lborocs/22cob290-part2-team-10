...
const active_employees = [
	{name:"John Gatsby", role:"Team member"}
];

const employee_list = ["Loki Dillon","Myrtle Blake","Lennox O'Moore","Danika Sellers","Alberto Dunkley","Ross Kouma","Perry Macgregor","Demi-Lee Hawes","Keagan Mccormick","Guto Guest",];
const roles = ["Team Leader","Team Member","Analyst","Consultant"];
...
function update_employees() {
	...
	var employee_select = $("#employees");
	var role_select = $("#roles");
...
function add_employee(eName, eRole) {
	...
	`<li class="list-group-item employee_list_item"><span class="employee_list_item_name">`+eName+`</span> 
	    ...
	    <select class="form-select emp_role_select">`+get_roles_as_select_menu(eRole)+`
...
function update_roles(){
	...
	employee_list_items.each(function() {
		...
		var selectmenu = $(this).first(".emp_role_select")[0];
		var role = $("option:selected", selectmenu).text();
...
$('#add_employee_btn').on('click', function (e) {
	var eName = $("#employees :selected").text();
	var eRole =  $("#roles :selected").text();
